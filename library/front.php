...
function kopa_front_enqueue_scripts() {
    if (!is_admin()) {
        ...
        if (is_single()) {
            wp_enqueue_script('kopa-user-rating-script', $dir . '/js/kopa-user-rating.js', array('jquery'), null, true);
        }
    }
}
...
function kopa_breadcrumb() {
    ...
    if (is_main_query()) {
        ...
        $breadcrumb_home = '<span>' . __('You are here:', kopa_get_domain()) . '</span> <a href="' . home_url() . '">' . __('Home', kopa_get_domain()) . '</a>';
        $breadcrumb = '';
        ?>
...
function kopa_add_icon_home_menu($items, $args) {
    ...
    if ($args->theme_location == 'main-nav') {
        if ($args->menu_id == 'toggle-view-menu') {
            ...
            $homelink = '<li class="menu-home-icon' . ( is_front_page() ? ' current-menu-item' : '') . '"><a href="' . home_url() . '">' . __('Home', kopa_get_domain()) . '</a><span></span></li>';
            $items = $homelink . $items;
        }
    }
    return $items;
